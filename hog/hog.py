...
def roll_dice(num_rolls, dice=six_sided):
    ...
    sum, i, currentRoll = 0, 0, 0
    pigOut = False

    while i < num_rolls:
        currentRoll = dice()
        if currentRoll == 1: #Pig Out
            pigOut == True
        else:
            sum += currentRoll
         i += i
...
def free_bacon(score):
    ...
    while pi > pow(10, score + 1) or pi < pow(10, score):
        pi //= 10
...
def swine_align(player_score, opponent_score):
    ...
    if player_score == 0 or opponent_score == 0:
        return False
    div = min(player_score, opponent_score)
    while div > 9:
        if player_score % div == 0 and opponent_score % div == 0:
            return True
        div -= 1
    return False
...
def announce_highest(who, last_score=0, running_high=0):
    ...
    def say(score0, score1):
        ...
        if leader=0 and score0-last_score > running_high:
            print(running_high, "points! The most yet for Player ", who)
        else:
            print(running_high, "points!", "The most yet for Player ", who)
    return say
...
def make_averaged(original_function, trials_count=1000):
    ...
    def averaged(args*):
        total, x = 0, 0
        while x < trials_count:
            total, x = total + averaged(args*), x + 1
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    max_score, x = 0, 1
    while x<11:
        current_value = make_averaged(roll_dice, trials_count)
...
def extra_turn_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    """
    # BEGIN PROBLEM 11
    if extra_turn == True or free_bacon(opponent_score) >= cutoff:
        return 0
    else:
        return num_rolls
    # END PROBLEM 11
